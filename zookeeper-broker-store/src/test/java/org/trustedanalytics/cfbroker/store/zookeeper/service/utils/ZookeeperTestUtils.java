 ...
 */
package org.trustedanalytics.cfbroker.store.zookeeper.service.utils;
...
public class ZookeeperTestUtils {
    ...
    public static void saveBytesInZNode(ZookeeperCredentials credentials, String path,
    ...
    public static void saveChildrenZNodes(ZookeeperCredentials credentials, String path,
        Map<String, byte[]> childrenToCreate) throws Exception {

        CuratorFramework tempClient = getNewTempClient(credentials);
        for (Map.Entry<String, byte[]> child : childrenToCreate.entrySet()) {
            tempClient.create()
                .creatingParentsIfNeeded()
                .forPath(path + "/" + child.getKey(), child.getValue());
        }
